...
def model_worker():
    ...
    while True:
        ...
        try:
            # Perform detection
            results = model(frame, conf=0.3, verbose=False)  # show=False for stability
            result_queue.put((results[0], frame))  # Store both result and frame
        except Exception as e:
            print(f"Inference error: {str(e)}")
            result_queue.put((None, None))  # Put None to avoid blocking
    ...
    try:
        while True:
            ...
            if not result_queue.empty():
                result, processed_frame = result_queue.get()
                if result is None or processed_frame is None:  # Skip if inference failed
                    continue
                ...
                for bbox, confi, cls in zip(bboxes, confidences, classes):
                    ...
                    if object_name.lower() == 'black-citrus-aphid' and can_send_telegram:
                        ...
                        try:
                            cv2.imwrite(temp_path, frame_bgr[y:y2, x:x2])
